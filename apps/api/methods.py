...
def monitor_list(request, json_rq):
    ...
    for monitor in Monitor.objects.filter(created_by=request.profile).order_by("id"):
        ...
        d["monitor_last_response_time"] = str(monitor.last_response_time.strftime("%S") if monitor.last_response_time else "")
        d["monitor_creation_date"] = str(monitor.creation_date.strftime("%d.%m.%Y") if monitor.creation_date else "")
        monitors.append(d)
...
def monitor_activity(request, validated_data):
    ...
    monitor_activities = []
    for monitor_activity in MonitorActivity.objects.filter(monitor_id=monitor_id)[:30]:
        d = {}
        ...
        d["monitor_response_time"] = str(monitor_activity.response_time.strftime("%S") if monitor_activity.response_time else "")
